...
Last.fm has no playlists, so this adapter never creates or edits one:
...
_NO_PLAYLISTS = "Last.fm has no playlists, so it cannot receive playlist changes"
...
class LastfmTarget(MirrorTarget):
    ...
    # No playlists at all, which keeps this provider out of playlist mirroring
