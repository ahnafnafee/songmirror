...
- 📻 **Last.fm**: Loved Tracks、Top Tracks（6 期間）、Recent Scrobbles を接続済みの任意のサービスへ送り出し、アカウントを認証すれば他サービスのお気に入り曲を **Last.fm 側へ** 戻せます。API にプレイリストがないため、ログイン済みのウェブセッションが必要で、それがなければプレイリストの送り先になることはありません。
...
Last.fm の API にはプレイリストがないため、既定ではプレイリストの送り先になりません。アカウント画面でログイン済みのウェブリクエストを貼り付けると、プレイリストも同期できます。**再生履歴** を読み取り専用のコレクションとして提供し、その **Loved Tracks** は他のどの提供元とも同じお気に入り曲のコレクションで、アカウントを認証すると書き込み可能になります。
...
- **お気に入り曲は常にネイティブの経路を使います。** お気に入り曲を Last.fm *へ* 同期すると、ウェブセッションを設定していても、書き込み先は Loved Tracks コレクションであってプレイリストではありません。
