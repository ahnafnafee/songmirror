...
- 📻 **Last.fm**: Loved Tracks、Top Tracks（6 期間）、Recent Scrobbles を接続済みの任意のサービスへ送り出し、アカウントを認証すれば他サービスのお気に入り曲を **Last.fm 側へ** 戻せます。プレイリストを持たないため、プレイリストの送り先になることはありません。
...
SongMirror はストリーミングカタログからの FLAC 抽出を自動化しません。新しい音声ソースはまず [`docs/monochrome-flac-assessment.md`](../monochrome-flac-assessment.md) の条件を満たす必要があります。提供元が公開しバージョン管理されたダウンロードまたはエクスポートのエンドポイント、利用者単位の認可、恒久的な複製についての明示的な権利、地域・有効期限・オフライン利用に関する文書化された規定、そして DRM やアクセス制御を回避しないことです。ローカルミラーは、自分が所有しているか複製を許可されている内容にのみ使ってください。
...
Last.fm はプレイリストを持たないため、プレイリストの送り先になることはありません。**再生履歴** を読み取り専用のコレクションとして提供し、その **Loved Tracks** は他のどの提供元とも同じお気に入り曲のコレクションで、アカウントを認証すると書き込み可能になります。
