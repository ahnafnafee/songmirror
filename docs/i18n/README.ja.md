...
- 📻 **Last.fm**: Loved Tracks、Top Tracks（6 期間）、Recent Scrobbles を接続済みの任意のサービスへ送り出し、アカウントを認証すれば他サービスのお気に入り曲を **Last.fm 側へ** 戻せます。プレイリストの同期には対応していないため、プレイリストの送り先になることはありません。
- 💿 **ローカル ダウンロード ミラー** — オフライン オーディオを保持します。Jellyfin の `AlbumArtist/Album` レイアウトでプレイリストごとに 1 つのフォルダーがあり、カバーと自動更新される `.m3u8` が含まれます。 既存の音楽ライブラリを指定すると、一致した曲は **元の品質のまま** コピーされ（FLAC は FLAC のまま）、ダウンロードは足りない分だけを補います。
...
- **Spotify 以外の任意のソース**: ミラーは同期がソースとして使うサービスを読み取ります。Spotify のプレイリストは従来どおりカタログ URL から取得し、それ以外のソース（Last.fm を含む）はアーティストと曲名で検索します。
- **まず自分のライブラリ（本物の FLAC）**: `LOCAL_LIBRARY_DIR` を既存の音楽ツリーに向けると、そこで見つかった曲は **元の形式のままプレイリストのフォルダーへコピー** されるため、FLAC は FLAC のままです。ローカルに一致がない曲だけが spotDL に回ります。照合はまず ISRC タグ、次に曲名とアーティストで行い、同期エンジンと同じ規則を使います。ハードリンクではなくコピーにしているのは意図的です。ミラーは更新日時を書き込みタグを補うため、ハードリンクだと元のファイルを書き換えてしまいます。空欄は無効を意味します。
- オーディオ品質 — ソースは YouTube なので、YT Music Premium Cookie がなければ、上限は ~128 ～ 160 kbps です。 `LOCAL_MIRROR_FORMAT=opus` は、mp3 再エンコードせずに YouTube のネイティブ ストリームを保持します。 Premium Cookie (`LOCAL_MIRROR_COOKIE_FILE`) は 256 kbps AAC のロックを解除します。 `flac` を選択すると出力コンテナが変更されますが、非可逆ソースを可逆オーディオに変換することはできません。 本当のロスレス音声が必要な場合は `LOCAL_LIBRARY_DIR` を使ってください。

SongMirror はストリーミングカタログからの FLAC 抽出を自動化しません。新しい音声ソースはまず `docs/monochrome-flac-assessment.md` の条件を満たす必要があります。提供元が公開しバージョン管理されたダウンロードまたはエクスポートのエンドポイント、利用者単位の認可、恒久的な複製についての明示的な権利、地域・有効期限・オフライン利用に関する文書化された規定、そして DRM やアクセス制御を回避しないことです。ローカルミラーは、自分が所有しているか複製を許可されている内容にのみ使ってください。
...
| Last.fm | API キーもセッションキーも期限切れにならないため、更新は行われません。Last.fm のアカウント設定でアプリのアクセスを取り消した場合にのみ、再認証してください。 |
...
2. 開発時は**テスト**モードを使用できます。OAuth 同意画面で Google アカウントをテストユーザーに追加し、7 日後にリフレッシュトークンの有効期限が切れたら再接続してください。長期間の無人運用には**本番環境**への切り替えも選択できます。[Google のトークン有効期限に関する規則](https://developers.google.com/identity/protocols/oauth2#expiration)を参照してください。
...
Last.fm はプレイリストの同期には対応していないため、プレイリストの送り先になることはありません。**再生履歴** を読み取り専用のコレクションとして提供し、その **Loved Tracks** は他のどの提供元とも同じお気に入り曲のコレクションで、アカウントを認証すると書き込み可能になります。

1. <https://www.last.fm/api/account/create> で API アカウントを作成します。**API キー** と **共有シークレット** の両方を控えてください。
2. アカウント → Last.fm で両方を貼り付け、続いて Last.fm の認証ページを進めます。

認証情報には 2 段階あり、書き込みを解放するのは 2 段階目です。

| 渡すもの | 得られるもの |
| --- | --- |
| API キー + ユーザー名 | そのプロフィールの公開範囲の読み取り |
| + 共有シークレット + 認証 | **非公開** の読み取りと、お気に入り曲の追加および解除 |

認証はブラウザーを一度通るだけです。SongMirror が Last.fm へ誘導し、アプリを承認すると、返されたトークンが **無期限のセッションキー** と交換されます。上の表にある貼り付け型の認証情報とは違い、再取得は一切不要です。Last.fm のアカウント設定から取り消せます。

これで利用できるようになるもの:

| コレクション | 内容 |
| --- | --- |
| **Loved Tracks** | お気に入り曲のコレクション。常に読み取り可能で、認証後は **書き込み可能** になるため、Spotify や TIDAL、Apple のお気に入りを Last.fm *へ* 同期できます |
| **Top Tracks（7 日 … 全期間）** | 順位付けされた読み取り専用のコレクションが 6 つ、Last.fm の期間ごとに 1 つ |
| **Recent Scrobbles** | 再生履歴。いま再生中の行は除きます |

頼りにする前に知っておきたい制限が 3 つあります。

- **ISRC がありません。** `user.*` のエンドポイントはアーティスト名と曲名だけを返すため、Last.fm の曲はカタログ上の同一性ではなく名前で照合されます。ISRC を持つ提供元なら避けられたはずの別バージョンとの誤一致が、ときどき起こります。お気に入りに追加する前に SongMirror は `track.getInfo` で Last.fm の正式な表記を問い合わせるので、わずかに異なる表記が 2 つ目の項目を作ることはありません。
- **順位付けのコレクションには日付がありません。** Top Tracks は曲ごとのタイムスタンプを持たないため、それらの曲は日付なしで同期され、追加日順を決めることはできません。
- **プレイリストの経路は使えません。** お気に入り曲を Last.fm *へ* 同期する場合はネイティブの経路を使う必要があります。名前付きプレイリストを作る経路が書き込める先は存在しません。

共有シークレットがない場合、指定するプロフィールは Loved Tracks と Top Tracks を公開しておく必要があります。
