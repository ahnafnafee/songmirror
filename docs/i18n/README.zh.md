...
- 📻 **Last.fm**：把你的 Loved Tracks、Top Tracks（六个时间段）和 Recent Scrobbles 同步到任意已连接的服务；授权账户后，还能把其他服务的喜欢歌曲同步回 **Last.fm**。它的 API 没有歌单，因此需要一个已登录的网页会话；没有会话时它永远不会成为歌单目标。
...
Last.fm 的 API 没有歌单，所以默认情况下它永远不会成为歌单目标；在账户页面粘贴一个已登录的网页会话即可同时同步歌单。它以只读集合的形式提供你的**收听记录**，其中的 **Loved Tracks** 与其他所有服务的喜欢歌曲集合一样，在你授权账户后即可写入。
...
- **喜欢歌曲始终使用原生路线。** 把喜欢歌曲同步*进入* Last.fm 时，即使配置了网页会话，写入的也是它的 Loved Tracks 集合，而不是歌单。
