...
- 📻 **Last.fm**: Loved Tracks, Top Tracks(여섯 기간), Recent Scrobbles를 연결된 어떤 서비스로든 내보내고, 계정을 승인하면 다른 서비스의 좋아하는 트랙을 **Last.fm 쪽으로** 되돌립니다. 재생목록 동기화를 지원하지 않으므로 재생목록 대상이 되는 일은 없습니다.
...
Last.fm은 재생목록 동기화를 지원하지 않으므로 재생목록 대상이 되는 일은 없습니다. **감상 기록**을 읽기 전용 모음으로 제공하며, 그중 **Loved Tracks**는 다른 모든 공급자와 마찬가지로 좋아하는 트랙 모음이고, 계정을 승인하면 쓰기가 가능해집니다.
...
- **재생목록 경로는 적용되지 않습니다.** 좋아하는 트랙을 Last.fm *으로* 동기화할 때는 기본 경로를 써야 합니다. 이름이 있는 재생목록을 만드는 경로가 쓸 대상이 없습니다.
