...
- 📻 **Last.fm**: Loved Tracks, Top Tracks(여섯 기간), Recent Scrobbles를 연결된 어떤 서비스로든 내보내고, 계정을 승인하면 다른 서비스의 좋아하는 트랙을 **Last.fm 쪽으로** 되돌립니다. API에 재생목록이 없어 로그인된 웹 세션이 필요하며, 그것이 없으면 재생목록 대상이 되지 않습니다.
- 💿 **로컬 다운로드 미러** — Jellyfin의 `AlbumArtist/Album` 레이아웃에 재생 목록당 하나의 폴더, 표지 및 자동 업데이트 `.m3u8`가 포함된 오프라인 오디오를 유지합니다. 기존 음악 라이브러리를 지정하면 일치하는 트랙이 **원래 품질 그대로** 복사되고(FLAC은 FLAC으로 유지), 내려받기는 빈자리만 채웁니다.
...
- **Spotify만이 아닌 모든 원본**: 미러는 동기화가 원본으로 사용하는 서비스를 읽습니다. Spotify 재생목록은 여전히 카탈로그 주소로 내려받고, 그 밖의 원본(Last.fm 포함)은 아티스트와 곡명으로 검색합니다.
...
| Last.fm | API 키도 세션 키도 만료되지 않으므로 갱신되는 것이 없습니다. Last.fm 계정 설정에서 앱 접근 권한을 철회한 경우에만 다시 승인하세요. |
...
Last.fm API에는 재생목록이 없어 기본적으로 재생목록 대상이 되지 않습니다. 계정 페이지에 로그인된 웹 요청을 붙여넣으면 재생목록도 동기화할 수 있습니다. **감상 기록**을 읽기 전용 모음으로 제공하며, 그중 **Loved Tracks**는 다른 모든 공급자와 마찬가지로 좋아하는 트랙 모음이고, 계정을 승인하면 쓰기가 가능해집니다.

1. <https://www.last.fm/api/account/create>에서 API 계정을 만드세요. **API 키**와 **공유 시크릿** 둘 다 적어 두세요.
2. 계정 → Last.fm에서 둘을 붙여넣고, 이어서 Last.fm 승인 페이지를 진행하세요.

자격 증명에는 두 단계가 있고, 쓰기를 여는 것은 두 번째 단계입니다.

| 제공하는 것 | 얻는 것 |
| --- | --- |
| API 키 + 사용자 이름 | 해당 프로필의 공개 범위 읽기 |
| + 공유 시크릿 + 승인 | **비공개** 읽기와 좋아하는 트랙 추가 및 해제 |

승인은 브라우저를 한 번 거치는 과정입니다. SongMirror가 Last.fm으로 보내고, 앱을 승인하면 돌아온 토큰이 **수명이 무한한 세션 키**로 교환됩니다. 위 표의 붙여넣기 자격 증명과 달리 다시 확보할 필요가 전혀 없습니다. Last.fm 계정 설정에서 철회할 수 있습니다.

이렇게 하면 다음을 쓸 수 있습니다.

| 모음 | 내용 |
| --- | --- |
| **Loved Tracks** | 좋아하는 트랙 모음. 항상 읽을 수 있고 승인 후에는 **쓸 수 있으므로** Spotify, TIDAL, Apple의 좋아요를 Last.fm *으로* 동기화할 수 있습니다 |
| **Top Tracks(7일 … 전체 기간)** | 순위가 매겨진 읽기 전용 모음 여섯 개, Last.fm 기간마다 하나 |
| **Recent Scrobbles** | 재생 기록. 지금 재생 중인 항목은 제외합니다 |

의존하기 전에 알아 둘 제한이 세 가지 있습니다.

- **ISRC가 없습니다.** `user.*` 지점은 아티스트와 곡명만 돌려주므로 Last.fm 트랙은 카탈로그 동일성이 아니라 이름으로 대조됩니다. ISRC를 가진 공급자라면 피했을 다른 버전과의 오매칭이 이따금 생깁니다. 좋아요를 누르기 전에 SongMirror는 `track.getInfo`로 Last.fm의 표준 표기를 물어보므로, 아주 비슷한 제목이 두 번째 항목을 만들지 않습니다.
- **순위 모음에는 날짜가 없습니다.** Top Tracks에는 트랙별 시각 정보가 없어서 그 트랙들은 날짜 없이 동기화되고 추가 순서를 정할 수 없습니다.
- **좋아하는 트랙은 항상 기본 경로를 사용합니다.** 좋아하는 트랙을 Last.fm *으로* 동기화하면 웹 세션이 설정되어 있어도 재생목록이 아니라 Loved Tracks 모음에 기록됩니다.

공유 시크릿이 없으면 지정한 프로필의 Loved Tracks와 Top Tracks가 공개되어 있어야 합니다.
