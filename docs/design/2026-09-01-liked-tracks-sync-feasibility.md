...
|---|---|---:|---:|---:|---|---|
| Spotify | Saved Tracks / Liked Songs | Yes | Yes | Yes | OAuth scopes `user-library-read` and `user-library-modify`. `GET /me/tracks` uses offset pagination, at most 50 per page, and returns `added_at`. Generic `PUT`/`DELETE /me/library` accepts at most 40 URIs. | Best-supported implementation. Existing OAuth users must reconnect because SongMirror currently requests playlist scopes only. |
...
2. Implement Spotify Saved Tracks first and require OAuth reconnection for the two library scopes.
