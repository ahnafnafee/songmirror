...
| TIDAL | Favorite Tracks | Yes | Yes | Yes | OAuth authorization-code + PKCE with `collection.read` and `collection.write`. Cursor pagination; default order is descending `addedAt`; add/remove accepts 1–50 IDs. The ordinary web-player token SongMirror can import is playlist-scoped and cannot substitute for these collection scopes. | Technically excellent with developer OAuth, including optional `addedAt` on additions, but public distribution needs written TIDAL approval. |
