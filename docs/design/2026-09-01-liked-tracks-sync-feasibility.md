...
| TIDAL | Favorite Tracks | Yes | Yes | Yes | The imported web-player grant carries `r_usr` and `w_usr` and uses the JSON:API collection relationship. Its captured refresh token renews the short-lived access token; cursor pagination defaults to descending `addedAt`, and add/remove accepts 1–50 IDs. | Read/add/remove and refresh behavior are implemented without a developer app. The first-party token contract can change, and public distribution still needs written TIDAL approval. |
...
| Amazon Music | Liked Tracks | Yes | Yes | Private only | Public `GET /me/tracks` uses `music::favorites:read`, max 100/page; `PUT /me/tracks/{id}` uses `music::favorites`, but documents only `LIKE` and `DISLIKE`. The first-party GraphQL schema exposes `setTrackLikeState(trackId, likeState)`, including `NEUTRAL`. | Do not map removal to `DISLIKE`. SongMirror gates exact likes behind its private web backend and uses `NEUTRAL` for unlike. |
...
- **TIDAL.** The official [OpenAPI document](https://tidal-music.github.io/tidal-api-reference/tidal-api-oas.json) defines `GET /userCollectionTracks/me` and `GET`/`POST`/`DELETE /userCollectionTracks/me/relationships/items`, cursor and sort behavior, the 50-item mutation ceiling, and relationship metadata containing `addedAt`. TIDAL's official auth SDK refreshes a user grant through `auth.tidal.com/v1/oauth2/token` using its saved client ID, refresh token, and scopes. SongMirror imports those values from the web player's sign-in response and uses its first-party `r_usr`/`w_usr` grant. TIDAL's [Developer Guidelines](https://developer.tidal.com/documentation/guidelines/guidelines-developer-guidelines) list transfer of data to another service as prohibited without express written approval.
