...
export function FolderField({ label, value, onChange, defaultPath = '', help, disabled = false }: {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-1.5">
      {editing ? (
        <TextField label={label} value={displayPath(value)} disabled={disabled} dir="ltr"
          placeholder={displayPath(defaultPath) || t("Full folder path")} help={help}
        ...
        <>
          ...
          <button id={id} type="button" disabled={disabled} aria-describedby={`${id}-help`}
            className={`${FIELD_INPUT_CLASSES} flex items-center gap-2 text-start`} onClick={() => void choose()}>
            <LuFolderOpen className="size-4 shrink-0 text-accent" aria-hidden="true" />
            <span className="min-w-0 flex-1 truncate">{displayPath(value || defaultPath) || t("Choose a folder…")}</span>
          </button>
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs font-semibold text-accent">
        <button type="button" className="min-h-8" onClick={choose} disabled={disabled}>{t("Browse…")}</button>
        <button type="button" className="min-h-8" onClick={() => setEditing(!editing)} disabled={disabled}>
          {editing ? t("Use folder picker") : t("Enter path manually")}
        </button>
      </div>
      {config?.scope === 'container' && <p className="text-xs text-text-3">{t("Docker storage: use a shared folder's path. Other folders on your computer must be shared with SongMirror first.")}</p>}
      {open && <FolderPicker title={t("Choose folder: {{label}}", { label })} initialPath={value || defaultPath}
