...
async function main() {
  ...
  try {
    ...
    await folder.click()
    await page.getByRole('dialog', { name: 'Choose folder: Backup folder' }).waitFor()
    ...
    await page.getByRole('dialog', { name: 'Choose folder: Download folder' }).waitFor()
    const dialog = page.getByRole('dialog', { name: 'Choose folder: Download folder' })
